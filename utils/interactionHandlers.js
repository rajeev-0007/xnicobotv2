...
/**
 * Legacy entry point for welcomer/leave/canvas panel interactions.
 *
 * The welcomer panel is owned by commands/automation/welcomer.js, which index.js
 * routes to first. This used to hold a second, complete welcomer UI (~580 lines
 * here plus ~420 lines of modal handlers and two panel builders) covering the
 * `welcomer_setup_*`, `welcomer_comp_*`, `welcomer_toggle_*` and `leave_setup_*`
 * families.
 *
 * All of it was unreachable. Every one of its 38 custom IDs was either rendered
 * by nothing at all, or rendered but claimed by welcomer.js first — and the only
 * path that could have reached it called deferUpdate() beforehand, so any
 * reply()/showModal() in here threw 40060 anyway.
 *
 * That dead copy caused a real outage rather than merely being clutter: it was
 * the sole implementation of /leave-setup's Set Channel, Set Message and Enable
 * buttons, which is why three of that command's four buttons did nothing.
 * /leave-setup now opens welcomer.js's own leave panel instead.
 *
 * Kept as a stub because panels already posted in servers still carry the old
 * components; those get an explicit notice rather than silent failure.
 */
async function handleWelcomerButtons(interaction) {
    ...
    try {
        await interaction.reply({
            content: '<:Cancel:1521227723916181644> This welcomer panel is outdated. Run `/welcomer` again to open the current panel.',
...
/**
 * Legacy entry point for anti-nuke panel interactions.
 *
 * The panel is owned by commands/utility/antinuke.js, which index.js now routes
 * to first. This remains only as a safety net for any call path that still
 * reaches here, and simply delegates.
 *
 * It previously carried ~90 lines of duplicated fallback handlers, including its
 * own hardcoded copy of the protection key list. That copy was one of the eleven
 * that made adding a protection a multi-file change (and silently dropped
 * whatever it did not know about) — it is now gone.
 */
async function handleAntiNukeButtons(interaction) {
    ...
    if (antinukeCmd?.handleInteraction) {
        try {
            if (await antinukeCmd.handleInteraction(interaction)) return;
        } catch (error) {
            log.error('Anti-Nuke Interaction Error:', error);
            if (!interaction.replied && !interaction.deferred) {
                await interaction.reply({
                    content: '<:Cancel:1521227723916181644> There was an error processing the Anti-Nuke interaction.',
                    flags: MessageFlags.Ephemeral
                }).catch(() => {});
            }
            return;
    ...
    if (!interaction.replied && !interaction.deferred) {
        await interaction.reply({
            content: '<:Cancel:1521227723916181644> This anti-nuke panel is outdated. Run `/antinuke` again.',
            flags: MessageFlags.Ephemeral
        }).catch(() => {});
    }
}
...
async function handleModalSubmit(interaction) {
    ...
    if (interaction.customId === 'embed_modal_basic') {
        const data = embedData.get(key) || {};
        const messageKey = `embed-${key}`;
 ...
 *
 * This used to be one of FOUR independent implementations supporting different
 * placeholder sets. This one (~54 placeholders) backed the runtime send, while
 * the welcomer preview used a different one (~33) that additionally supported
 * {separator}. The result was a preview that rendered dividers which then
 * arrived literally in real messages. Re-exported here so the many existing
