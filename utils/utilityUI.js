 ...
 * Utility UI Toolkit
 * ──────────────────────────────────────────────────────────────────────────
 * Minimal, professional Components V2 panels for the `utility` category.
 * Mirrors the other category toolkits. Like ownerUI, the helpers return a
 * COMPLETE reply payload (`{ components, flags }`) so call sites convert with
 * a single inline `require('../../utils/utilityUI')` — no per-file import
 * edits. The global reply patcher applies the guild accent + footer when sent.
 */
...
module.exports = { E, errReply, warnReply };
