...
async function handleDeposit(reply, userId, args, guildId) {
    ...
    if (amount === undefined || amount === null || isNaN(amount) || amount <= 0) {
        ...
        addTextDisplay(container, [
            '# <:Invoice:1521227903956811836> Deposit',
            '',
            '**Usage:** `deposit <amount | all>`',
            '',
            '**Your Balances:**',
            `${coinIcon(guildId)} Wallet: ${formatCoinsAmount(userData.coins, guildId)}`,
            `<:Invoice:1521227903956811836> Bank: ${formatCoins(userData.bank, guildId)}`,
            '',
            '**Examples:**',
            '`deposit 500`',
            '`deposit all`',
    ...
    if (amount > userData.coins) {
        ...
        addTextDisplay(c, '<:Cancel:1521227723916181644> You don\'t have that many coins in your wallet!');
    ...
    const container = createContainer(0xCAD7E6);
    addTextDisplay(container, [
        '# <:Invoice:1521227903956811836> Deposit Successful',
        '',
        `<:Checkedbox:1521227734943269077> Deposited **${formatCoins(amount, guildId)}**.`,
    ].join('\n'));
    ...
    addTextDisplay(container, [
        ...
        `${coinIcon(guildId)} Wallet: ${formatCoinsAmount(userData.coins, guildId)}`,
        `<:Invoice:1521227903956811836> Bank: ${formatCoins(userData.bank, guildId)}`,
        `<:Invoice:1521227903956811836> **Total:** ${formatCoins(userData.coins + userData.bank, guildId)}`,
    ].join('\n'));
