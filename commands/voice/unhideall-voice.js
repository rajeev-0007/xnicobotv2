...
module.exports = {
    ...
    category: 'voice',
    aliases: ['unhidevc', 'unhideallvc', 'vuha'],
    ...
    async executePrefix(message, args) {
        ...
        let done = 0;
        for (const [, ch] of voiceChannels) { try { await ch.permissionOverwrites.edit(roleId, { ViewChannel: null }); done++; } catch {} }

        return message.reply(vui.payload(vui.ok(gid, 'Voice Channels Unhidden', `Unhid **${done}/${voiceChannels.size}** voice channels for **${roleName}**.`, {
            Unhidden: `${done}/${voiceChannels.size}`, Role: roleName, Effect: 'Can see channels again', Moderator: message.author.username
        }, vui.E.unlock)));
    }
};
