...
window.__previewMsg = function() {
    ...
    if (!preview) return;
    const previewCfg = JSON.parse(JSON.stringify(window.__mbWorking || {}));
    
    // Replace variables for realistic preview
    const g = state.currentGuild;
    if (g) {
        ...
        if (previewCfg.description) previewCfg.description = previewCfg.description.replaceAll('{user}','@User').replaceAll('{username}','User').replaceAll('{server}',g.name).replaceAll('{membercount}','42').replace(/\{separator[^}]*\}/g,'───');
        if (previewCfg.footer) previewCfg.footer = previewCfg.footer.replaceAll('{membercount}','42');
    }

    preview.innerHTML = buildDiscordPreview(previewCfg, state.botInfo);
};
...
function _persistMbDraft() {
    ...
    if (typeof window.__previewMsg === 'function') window.__previewMsg();
}
...
function _renderMbBody(g, w) {
    ...
    const html = `
        ...
        <!-- PREVIEW -->
        <div class="card mb-2">
            <div class="card-h"><div class="ic">${icon('chat')}</div><div class="tt"><div class="t">Live Preview</div><div class="s">Approximate rendering of the message.</div></div></div>
