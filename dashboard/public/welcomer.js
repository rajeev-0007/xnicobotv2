        ...
        <div class="card mt-2 mb-2" id="welc-preview-card" ${vis(w.enabled)}>
            ...
            <div class="preview" id="welc-preview">
                ...
                <div class="embed" id="welc-prev-embed" style="border-left-color:${esc(w.color||'#bcf1e4')}">
                    ${w.title?`<div class="t">${renderDiscord(w.title)}</div>`:''}
                    <div class="d">${renderDiscord(w.content||'Welcome @User to **Server**!')}</div>
                    ${w.footer?`<div style="margin-top:.4rem;font-size:.75rem;color:#72767d">${renderDiscord(w.footer)}</div>`:''}
                </div>
            </div>
        </div>
